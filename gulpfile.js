 ...
 * 
 * This script is for building some of my extra web projects from its sources; For that, the following operations can be done:
...
if (process.versions.node <= '0.12.0') {
    ...
    console.warn('gulp: recommand node version 0.12.x or later ') ;
...
gulp.task('sass', function() {
    ...
    sass( paths.sass, {sourcemap: true, 
                       style: 'expanded',
                       loadPath: [ paths.bower+"/Materialize/sass" ]
                      } 
    ...
    notifier.notify({ title: 'sass:', message: 'css generation\'s / minification\'s task complete!' }) ;
...
gulp.task('watch', function() {
    ...
    notifier.notify({ title: 'watcher:', message: 'source files are being watched!' }) ;
});
